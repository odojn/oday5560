...
import { ShoppingCart, Printer, CheckCircle, Search, Barcode, Zap } from 'lucide-react';
...
export default function WholesalePOS() {
  ...
  // All products for wholesale
  const storeProducts = products.filter(p => p.storeId === storeId && p.quantity > 0);
  ...
  const totalAmount = cart.reduce((acc, curr) => acc + (curr.product.wholesalePrice * curr.qty), 0);
  ...
  const handleCheckout = (printMode: 'NONE' | 'MODAL' | 'DIRECT' = 'MODAL') => {
    ...
    const finalCustomerName = selectedCustomer ? selectedCustomer.name : (customCustomerName || 'عميل جملة نقدي');
    ...
    const saleRecord = {
      ...
      paidAmount: paid,
      debtAmount: debt,
      items: cart.map(c => ({ productId: c.product.id, quantity: c.qty, price: c.product.wholesalePrice })),
      totalAmount,
    ...
    if (debt > 0) {
      addDebtRecord({
        ...
        status: 'UNPAID',
        notes: `دين ناتج عن فاتورة بيع جملة #${saleId.slice(0, 8)}`,
        createdAt: date
      });
    }
      ...
      <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col overflow-hidden">
        <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <h4 className="font-bold text-slate-800">المنتجات (الجملة)</h4>
            <Button 
              size="sm" 
              onClick={() => setIsScannerOpen(true)}
              className="bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200 text-xs flex items-center gap-1.5"
            >
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="relative">
              <Barcode className="w-4 h-4 absolute right-3 top-3 text-emerald-600" />
              <input
                type="text"
                placeholder="مسح كود/باركود (Enter)..."
                className="h-10 pl-3 pr-9 border border-emerald-200 rounded-lg text-xs bg-emerald-50/40 focus:bg-white focus:border-emerald-600 outline-none w-48 ltr font-mono"
                value={quickBarcode}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
            {filteredProducts.map(p => (
              <div 
                key={p.id} 
                onClick={() => addToCart(p)}
                className="bg-white border border-slate-100 rounded-xl p-3 cursor-pointer hover:border-emerald-400 hover:shadow-sm transition-all flex flex-col text-center group"
              >
                {p.imageUrl ? (
                ...
                <p className="text-emerald-600 font-mono font-bold mt-auto">{formatCurrency(p.wholesalePrice, storeSettings.currency)}</p>
                <p className="text-[10px] text-slate-400 mt-1 uppercase">الكمية: {formatNumber(p.quantity)}</p>
              </div>
            ))}
            {filteredProducts.length === 0 && (
              <div className="col-span-full py-12 text-center text-slate-400 text-sm">لا يوجد منتجات متاحة في المخزون</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col overflow-hidden">
        ...
        <div className="p-4 border-b border-slate-100 space-y-3 bg-slate-50/30">
          <div>
            ...
            >
              <option value="">-- عميل جملة نقدي عام --</option>
          ...
          {!selectedCustomerId && (
            <Input 
              placeholder="أو اكتب اسم العميل..." 
            ...
            <div className="grid grid-cols-3 gap-1">
              <button
                ...
                className={`py-1.5 px-2 text-xs font-bold rounded-lg border transition-all ${
                  paymentType === 'CASH' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-slate-700 border-slate-200'
                }`}
              >
                نقداً
              </button>
              <button
                ...
                className={`py-1.5 px-2 text-xs font-bold rounded-lg border transition-all ${
                  paymentType === 'CARD' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-slate-700 border-slate-200'
                }`}
              >
                بطاقة
              </button>
          ...
          {cart.map(item => (
            <div key={item.product.id} className="flex flex-col gap-3 py-3 border-b border-slate-100 last:border-0">
              <div className="flex justify-between items-start">
                ...
                <span className="font-bold font-mono text-emerald-600 text-sm">{formatCurrency(item.product.wholesalePrice * item.qty, storeSettings.currency)}</span>
              </div>
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50/50 flex flex-col gap-3">
          <div className="flex justify-between items-center">
            ...
            <span className="text-xl font-bold font-mono text-emerald-600">{formatCurrency(totalAmount, storeSettings.currency)}</span>
          </div>
